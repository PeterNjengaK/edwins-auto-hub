window.Manager = (() => {
  ...
  const label = (c) =>
    c ? `${c.year} ${c.make} ${c.model}` : "Removed vehicle";
  const date = (value) =>
    value
  ...
  function recordSale(id) {
    ...
    show(
      form(
        ...
        `<p class="span-2">This updates the vehicle to Sold, closes this enquiry as Won, and cancels outstanding viewings for the car. No payment is collected.</p>${select("carId", "Vehicle", [["", "Choose a vehicle"], ...available.map((c) => [c.id, label(c)])], q.carId)}${input("price", "Final sale price (KSh)", "", "number", 'required min="1" max="1000000000"')}<label class="check-field span-2"><input type="checkbox" required>I confirm that this sale has been completed.</label>`,
        "Record sale",
      ),
    );
    bind(
      (b) => authenticated(`/api/enquiries/${id}/sale`, "POST", b),
      "Sale recorded and vehicle updated.",
    );
  }
  ...
  function enquiriesView(target, search) {
    ...
    target.innerHTML = `<div class="manager-section-head"><span>${result.length} enquiries</span><select id="enquiry-filter" aria-label="Filter enquiries">${["All", "Overdue", "Seller submissions", ...statusOptions].map((v) => `<option ${v === enquiryFilter ? "selected" : ""}>${v}</option>`).join("")}</select><a class="text-link" href="/api/admin/export/enquiries">${i("download")}Export CSV</a></div><div class="enquiry-list">${result.map((q) => `<article class="enquiry-item"><div class="enquiry-heading"><div><h3>${e(q.name)} ${badge(q.type === "sell" ? "Seller submission" : q.type === "vehicle" ? "Vehicle enquiry" : "General enquiry")}${q.priority === "High" ? badge("High priority", "priority-high") : ""}</h3><p>${date(q.createdAt)} &middot; ${e(q.vehicle || "General conversation")}</p></div><select data-enquiry-status="${q.id}" aria-label="Status for ${e(q.name)}" ${q.sale ? "disabled" : ""}>${statusOptions.map((v) => `<option ${v === q.status ? "selected" : ""}>${v}</option>`).join("")}</select></div><p class="enquiry-message">${e(q.message)}</p>${q.details ? `<p class="enquiry-details">${e(q.details.year)} ${e(q.details.make)} ${e(q.details.model)} &middot; ${money(q.details.price)} &middot; ${Hub.number(q.details.mileage)} km</p>` : ""}<div class="enquiry-contact"><a href="mailto:${e(q.email)}">${i("mail")}${e(q.email)}</a>${q.phone ? `<a href="tel:${e(q.phone)}">${i("phone")}${e(q.phone)}</a>` : ""}${q.followUp ? `<span class="${due(q) ? "due-date" : ""}">${i("calendar-clock")} Follow up ${e(q.followUp)}</span>` : ""}</div>${q.images?.length ? `<div class="enquiry-photos">${q.images.map((image, n) => `<img src="${e(image)}" alt="Seller photo ${n + 1}" loading="lazy">`).join("")}</div>` : ""}<div class="enquiry-actions"><button class="button outline small" data-manage-enquiry="${q.id}">${i("notebook-pen")}Notes & follow-up${q.notes?.length ? " (" + q.notes.length + ")" : ""}</button>${q.type === "sell" ? (q.convertedCarId ? `<button class="button outline small" data-edit="${q.convertedCarId}">${i("car-front")}Open vehicle draft</button>` : `<button class="button outline small" data-create-draft="${q.id}">${i("file-plus-2")}Create vehicle draft</button>`) : `<button class="button outline small" data-schedule="${q.id}">${i("calendar-plus")}Arrange viewing</button>${!q.sale ? `<button class="button outline small" data-record-sale="${q.id}">${i("badge-check")}Record sale</button>` : badge("Sale: " + money(q.sale.price))}`}<button class="icon-button" data-delete-enquiry="${q.id}" title="Delete enquiry" aria-label="Delete enquiry from ${e(q.name)}" ${q.sale ? "disabled" : ""}>${i("trash-2")}</button></div></article>`).join("") || empty("No enquiries match this view.")}</div>`;
  ...
  return {
    ...
    render(view, target, search) {
      ...
      else if (view === "reports") reports(target);
    ...
    enhance(view) {
      ...
      target.insertAdjacentHTML(
        "afterbegin",
        `<div class="manager-section-head"><span>${cars.length} vehicles in inventory</span><select id="publication-filter" aria-label="Filter publication">${["All", "Published", "Draft", "Archived"].map((v) => `<option ${inventoryFilter === v ? "selected" : ""}>${v}</option>`).join("")}</select><a class="text-link" href="/api/admin/export/inventory">${i("download")}Export CSV</a></div>`,
      );
